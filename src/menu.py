...
def bot_config_menu(player_name):
    pygame.event.clear()
    levels = ['Easy', 'Medium', 'Hard']  # Keep the level strings here
    algorithms = ['Minimax', 'AlphaBeta', 'MonteCarlo', 'Random']
    selected_algorithm_index = 0
    selected_level = 2  # Default to Medium
    ...
    while True:
        ...
        if draw_button(f"Algorithm: {algorithms[selected_algorithm_index]}", WIDTH // 2 - 150, 200, 300, 50):
            selected_algorithm_index = (selected_algorithm_index + 1) % len(algorithms)

        if algorithms[selected_algorithm_index] != "Random":
            if draw_button(f"Difficulty: {levels[selected_level]}", WIDTH // 2 - 150, 270, 300, 50):
                selected_level = (selected_level + 1) % len(levels)

        if draw_button("Confirm", WIDTH // 2 - 150, 390, 300, 50):
            return algorithms[selected_algorithm_index], levels[selected_level]  # Return difficulty as string
...
def cc_config_menu():
    ...
    selected_level1 = 2
    selected_level2 = 2
    selected_algorithm_index1 = 0
    selected_algorithm_index2 = 0
    ...
    while True:
        ...
        if draw_button(f"Algorithm: {algorithms[selected_algorithm_index1]}", WIDTH // 4 - 150, 170, 300, 50):
            selected_algorithm_index1 = (selected_algorithm_index1 + 1) % len(algorithms)

        # Only show difficulty button if the algorithm is not "Random"
        if algorithms[selected_algorithm_index1] != "Random":
        ...
        if draw_button(f"Algorithm: {algorithms[selected_algorithm_index2]}", 3 * WIDTH // 4 - 150, 170, 300, 50):
            selected_algorithm_index2 = (selected_algorithm_index2 + 1) % len(algorithms)

        # Only show difficulty button if the algorithm is not "Random"
        if algorithms[selected_algorithm_index2] != "Random":
        ...
        if draw_button("Confirm", WIDTH // 2 - 150, 320, 300, 50):
            return (algorithms[selected_algorithm_index1], selected_level1), (algorithms[selected_algorithm_index2], selected_level2)
